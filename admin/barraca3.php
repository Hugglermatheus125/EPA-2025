...
<html lang="pt-br">
    ...
    <body>
        <div class="mb-3">
            ...
            <div class="row"><h2 class="roboto-subtitle text-center mt-3">Barraca 3</h2></div>
            <div class="row"><p class="roboto-regular">Digite o ID do usuario para atualizar sua pontuação</p></div>
            <form action="Barraca3.php" class="roboto-regular" method="post">
...
if ($usuarioId && !$usarioPontosBarraca1) {
    ...
    if ($usarioNome) {
        ...
        */
        ...
        echo '<form method="post" action="Barraca3.php" class="roboto-regular mb-3">
...
if ($usuarioId && $usarioPontosBarraca1 !== null) {
    ...
    $stmtPontos = $pdo->prepare('SELECT rPontuacaoFinal, rParticipouBarraca3 FROM inforanking WHERE rIdParticipante = :aId');
    $stmtPontos->bindParam(':aId', $usuarioId);
    $stmtPontos->execute();
    $usarioPontosTotais = $stmtPontos->fetchColumn();
    ...
    if ($usarioPontosTotais !== false) {
        ...
        */
        ...
        // Atualiza o BD
        $stmtAtualizar = $pdo->prepare('UPDATE inforanking SET rPontuacaoFinal = :pontos, rParticipouBarraca3 = 1 WHERE rIdParticipante = :aId');
        $stmtAtualizar->bindParam(':pontos', $usarioPontosTotais);
        $stmtAtualizar->bindParam(':aId', $usuarioId);
        $stmtAtualizar->execute();

        echo '<p class="roboto-regular">Pontuação atualizada para: ' . $usarioPontosTotais . ' e agora participou da 3º Barraca</p>';
    } else {
